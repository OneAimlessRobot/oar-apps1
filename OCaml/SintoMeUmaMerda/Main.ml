...
let rec mostraFich path=
	show(BinTree.readTree path)
;;
let rec maxFich path=
	BinTree.max (BinTree.readTree path)
;;
...
let exec comm filename = (* falta apenas completar esta funcao *)
    match comm with
      | "mostra" -> mostraFich filename
      | "maximo" -> print_string (string_of_int (maxFich filename))
      | "ajuda" -> help ()
      | "sair" -> byeBye ()
      | _ -> help ()
