#include "../Includes/includesAndDefs.h"
...
int main(int argc, char *argv[])
{
    ...
    if (!g_Debug)
    {
        while ((std::cout << "Insere a pass se fores mais de 18" << std::endl) && (!(std::cin >> myInput) || myInput != (std::string) passwordToMatch))
        {
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // discard input
            std::cout << "passwordIncorreta " << passwordToMatch<< std::endl;
            g_Attempts++;
            if (g_Attempts >= 3)
            {
                return 1;
