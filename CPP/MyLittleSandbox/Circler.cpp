...
#include "Circler.hpp"
    ...
    void Circler::parametricEquationCalculation(){
        float newX=this->radius*cos((counter->getCurrent()*(M_PI/this->precision)))+this->xCenter;
        float newY=this->radius*sin((counter->getCurrent()*(M_PI/this->precision)))+this->yCenter;
