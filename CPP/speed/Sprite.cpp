...
#include "Sprite.hpp"
    ...
    Sprite::Sprite(SDL_Renderer*renderer,int x,int y,const char* pathToSprite,int unlimitted,int cycleNumber){
    ...
    SDL_Point Sprite::getPos(){
        ...
        return (SDL_Point){frames[0]->getPos().x+frames[0]->getWidth()*0.5,frames[0]->getPos().y+frames[0]->getHeight()*0.5};
