...
void handleEvents(SDL_Event event, const Uint8 *KEYBOARD,Jumper* jumper,double*fps,GameState &state);
...
int main(int argc, char* argv[]){
        ...
        ForceField*field=new ForceField(renderer,150,150,600,1,1,(SDL_Color){0,0,0,255},FFIELD_IMAGE_PATH,1,1,0,0);
//        ODESolver*solver=new ODESolver(0, 0, mode.w,mode.h,20,20,yFunc);
        ...
        while(aux::OnesAreContained(currentState.mask,RUN_MASK)){
                ...
                SDL_Point startPos=(SDL_Point){jumper->getPos().x,jumper->getPos().y};
            ...
            if(aux::OnesAreContained(currentState.mask,ARMY_MASK)){
                    if(button & SDL_BUTTON_LMASK){
                            SDL_Point point=(SDL_Point){currentState.mouseX1,currentState.mouseY1};
                            SDL_bool detect1=SDL_PointInRect(&point,army[counter->getCurrent()]->getBody());
                    if(detect1){
            ...
            SDL_Point endPos=(SDL_Point){jumper->getPos().x,jumper->getPos().y};
            SDL_Point mousePos=(SDL_Point){currentState.mouseX1,currentState.mouseY1};

            double mDist=aux::distBetPointsInts(currentState.mouseX1,currentState.mouseY1,currentState.mouseX2,currentState.mouseY2);

            double jDist=aux::distBetPoints(&startPos,&endPos);
            currentState.mSpeed=(int)(((double)mDist*0.5)/(double)(delta*0.05));

            currentState.jSpeed=(int)(((double)jDist*0.5)/(double)(delta*0.05));

                    if(aux::OnesAreContained(currentState.mask,ARMY_MASK)){

                        SDL_Point throwablePos=army[counter->getCurrent()]->getCenter();
                        if(army[counter->getCurrent()]->getValue()){
                        int speed=aux::distBetPoints(&mousePos,&throwablePos)*0.01*currentConstants.throwingSpeed,
                            deltaX=currentState.mouseX1-throwablePos.x,
                            deltaY=currentState.mouseY1-throwablePos.y;
                            GeometricVector<int>*updatedVec=new GeometricVector<int>(throwablePos.x,throwablePos.y,deltaX,deltaY,positive,speed);
...
//                    std::cout<<jumper->getCenter().x<<" , "<<jumper->getCenter().y<<"\n";
                ...
                solver->drawCurve(renderer,(SDL_FPoint){currentState.mouseX1,currentState.mouseY1},0.1,6000);
...
void listOperations(Pen*pen,Level* currentLevel,ForceField* field,std::vector<Throwable*> &army,Jumper*jumper,GameState &currentState,int thickness,SDL_Renderer*renderer){
            ...
            if(aux::OnesAreContained(currentState.mask,ARMY_MASK)){
                if(aux::OnesAreContained(currentState.mask,THROWABLE_ADD_MASK)){
                            ...
                            Throwable* added= new Throwable(renderer,currentState.mouseX1,currentState.mouseY1,50,(SDL_Color){255,0,0,120},50,10,THROWABLE_IMAGE_PATH,0,10,0.0625);
                        ...
                        while(!it2->isOver()){
                          ( (Planet*)(it2->getCurrent()))->addThrowable(added);
                            it2->increment();
