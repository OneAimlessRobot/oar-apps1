...
thread_tree_node* createNewThreadNode(int nsize,void* (*pfunc)(pausableFuncArgs*),void* args){
		...
		newFunc->args->twowaysem[0]=tree->twowaysem[0];
		newFunc->args->twowaysem[1]=tree->twowaysem[1];
		node->pfunc=newFunc;
...
thread_tree_node* generateWorkHorse(int nheight,int nsize,void* (*pfunc)(pausableFuncArgs*),void* args){
	if(!nheight){

		return createNewThreadNode(nsize,pfunc,args);

	}
	else{

		
		thread_tree_node* node= createNewThreadNode(nsize,pfunc,args);

		for(int i=0;i<nsize;i++){
		
			node->children[i]=generateWorkHorse(nheight-1,nsize,pfunc,args);
	...
	tree->root=generateWorkHorse(nheight,nsize,pfunc,args);
	tree->nsize=nsize;
...
void joinWorkHorse(thread_tree_node* tree){
...
void detachWorkHorse(thread_tree_node* tree){
...
void destroyWorkOnNode(thread_tree_node* tree){
...
void destroyWorkOnArray(thread_tree_node** array,int size){
