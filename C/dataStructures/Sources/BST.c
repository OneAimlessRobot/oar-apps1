...
static BSTNode* findNodeInBSTree(BSTree* tree,void* data){
	...
	while(result&&compare(data,result->mem,tree->elemSize)){
	if(compare(data,result->mem,tree->elemSize)<0){
	...
	else if(compare(data,result->mem,tree->elemSize)>0){
...
void destroyBSTreeAux(BSTNode* node){
